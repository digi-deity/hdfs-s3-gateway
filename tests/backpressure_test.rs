...
async fn start_gateway(
    ...
    let config = scope.config_with(max_concurrent, None);
...
async fn backpressure_503_slowdown() {
    ...
    write_file(&scope, "data/obj.txt", b"hello").await;

    // Cap at 2 in-flight requests; fire 50 concurrent GETs.
    let (addr, _handle) = start_gateway(&scope, 2).await;
...
async fn backpressure_holds_permit_during_body_streaming() {
    ...
    write_file(&scope, "data/big.txt", &payload).await;

    // Cap at exactly 1 in-flight request.
    let (addr, _handle) = start_gateway(&scope, 1).await;
    let base = format!("http://{addr}/hdfs");
    let http = HttpClient::builder()
        .pool_idle_timeout(Duration::from_secs(1))
        .build()
